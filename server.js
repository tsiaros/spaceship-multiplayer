...
let npcAutoSpawn = true;
let powerupAutoSpawn = true;
...
io.on('connection', (socket) => {
  ...
  socket.on('adminSetEnemySpawnCount', (count) => {
    if (!socket.isAdmin) return;
    enemySpawnCount = Math.max(1, Math.min(10, parseInt(count) || 1));
    socket.emit('adminActionSuccess', `Enemy spawn count set to ${enemySpawnCount}`);
    socket.emit('adminNPCSpawnCounts', { enemyCount: enemySpawnCount, bossCount: bossSpawnCount });
  });

  socket.on('adminSetBossSpawnCount', (count) => {
    if (!socket.isAdmin) return;
    bossSpawnCount = Math.max(1, Math.min(5, parseInt(count) || 1));
    socket.emit('adminActionSuccess', `Boss spawn count set to ${bossSpawnCount}`);
    socket.emit('adminNPCSpawnCounts', { enemyCount: enemySpawnCount, bossCount: bossSpawnCount });
  });

  socket.on('adminGetNPCSpawnCounts', () => {
    if (!socket.isAdmin) return;
    socket.emit('adminNPCSpawnCounts', { enemyCount: enemySpawnCount, bossCount: bossSpawnCount });
  });

  socket.on('adminMasterAutomatic', () => {
    if (!socket.isAdmin) return;
    npcAutoSpawn = true;
    powerupAutoSpawn = true;
    if (npcAutoSpawn) {
      lastEnemySpawn = Date.now();
      lastBossSpawn = 0;
    }
    ...
    socket.emit('adminActionSuccess', 'All automatic spawning activated');
  });

  socket.on('adminStopAll', () => {
    if (!socket.isAdmin) return;
    npcAutoSpawn = false;
    powerupAutoSpawn = false;
    ...
    // Clear all powerups
    powerups.forEach(pu => io.emit('powerupRemoved', { powerupId: pu.id }));
    powerups.clear();
    
    // Clear all firing mode pickups
    firingModes.forEach(fm => io.emit('firingModeRemoved', { modeId: fm.id }));
    firingModes.clear();
    ...
    socket.emit('adminNPCAutoStatus', { active: false });
    socket.emit('adminPowerupAutoStatus', { active: false });
    socket.emit('adminClockStatus', { mode: 'off' });
    socket.emit('adminXStatus', { mode: 'off' });
    socket.emit('adminActionSuccess', 'All spawning stopped and map cleared');
  });
...
setInterval(() => {
  ...
  if (now - lastEnemySpawn >= spawnRate) {
    const totalEnemies = enemySpawnCount * playerCount;
  ...
  if (lastBossSpawn > 0 && now - lastBossSpawn >= 180000) {
    const totalBosses = bossSpawnCount * playerCount;
