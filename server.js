// server.js - Complete Multiplayer Server with ALL fixes
...
io.on('connection', (socket) => {
  ...
  // Send full game state immediately on connect
  ...
  socket.on('join', (data) => {
    ...
    const player = {
      ...
      health: 6, kills: 0, shipSize: 'small', lastDamageTime: Date.now()
    };
  ...
  socket.on('shoot', (data) => {
    ...
    const bullet = { id: bulletId, ...data, color: player.color, owner: socket.id, createdAt: Date.now() };
...
setInterval(() => {
  const now = Date.now();
  players.forEach(p => {
    if (!p.name.startsWith('SPEC_') && p.health < 6 && (now - p.lastDamageTime) >= 120000) {
      p.health = 6;
      io.emit('playerHealthRegen', { playerId: p.id, health: 6 });
    }
  });
}, 5000);
...
// Clock Attack - Horizontal sweep left to right
setInterval(() => {
  ...
  // 0° = horizontal left, 90° = straight up, 180° = horizontal right
  // We want bullets to go UP and ACROSS the horizon
  ...
  const bullet = {
    ...
    x: startX,
    y: startY,
    vx: Math.cos(radians) * 8,  // Horizontal component
    vy: -Math.sin(radians) * 8, // Vertical component (negative = up)
    color: '#ff0000',
    owner: 'clock'
  };
...
// Periodic state sync - prevents desyncs
setInterval(() => {
  // Broadcast full player state to prevent ghost ships
  ...
  // Force remove any ghost NPCs
  const npcIds = Array.from(npcs.keys());
  io.emit('npcListSync', npcIds);
}, 2000); // Every 2 seconds

// Admin state broadcast - keep admin panel updated
setInterval(() => {
